...
const mealCatalog = mealCatalogJson as CookunityMealCatalogEntry[];
...
function inferAthleteBoost(meal: CookunityMealCatalogEntry, haystack: string) {
  let score = 0;
  const tags = meal.dietaryTags.map(normalizeTag);
  if (tags.includes("high protein")) score += 30;
  if (tags.includes("low sugar")) score += 10;
  if (haystack.includes("salmon")) score += 10;
  if (haystack.includes("chicken")) score += 10;
  if (haystack.includes("steak")) score += 8;
  if (haystack.includes("turkey")) score += 8;
  if (haystack.includes("quinoa")) score += 7;
  if (haystack.includes("rice")) score += 6;
  if (haystack.includes("potato")) score += 6;
  if (haystack.includes("greens")) score += 4;
  return score;
}

function scoreMeal(
  meal: CookunityMealCatalogEntry,
  keyword: string,
  secondaryKeywords: string[],
  filters: string[],
  slot: "lunch" | "dinner" | "any",
  ...
  if (slot === "lunch") {
    if (lunchIndicators.some((indicator) => haystack.includes(indicator))) score += 18;
    if (dinnerIndicators.some((indicator) => haystack.includes(indicator))) score += 4;
  } else if (slot === "dinner") {
    if (dinnerIndicators.some((indicator) => haystack.includes(indicator))) score += 18;
    if (lunchIndicators.some((indicator) => haystack.includes(indicator))) score += 4;
  }
  ...
  if (keyword.toLowerCase().includes("meal plan")) {
    if (haystack.includes("balanced")) score += 8;
    if (haystack.includes("vegetable")) score += 5;
    if (haystack.includes("weeknight")) score += 5;
  }
...
export function inferCookunityMealFilters(keyword: string) {
  const lowered = keyword.toLowerCase();
  const filters: string[] = [];
  if (lowered.includes("keto")) filters.push("keto");
  if (lowered.includes("vegan")) filters.push("vegan");
  if (lowered.includes("vegetarian")) filters.push("vegetarian");
  if (lowered.includes("gluten free")) filters.push("gluten free");
  if (lowered.includes("dairy free")) filters.push("dairy free");
  if (lowered.includes("low sodium")) filters.push("low sodium");
  if (lowered.includes("low carb")) filters.push("low carb");
  if (lowered.includes("paleo")) filters.push("paleo");
  if (lowered.includes("mediterranean")) filters.push("mediterranean");
  if (lowered.includes("athlete") || lowered.includes("high protein")) filters.push("high protein");
...
export function searchCookunityMeals(options: CookunityMealSearchOptions) {
  const {
    keyword,
    secondaryKeywords = [],
    filters = inferCookunityMealFilters(keyword),
    count = 12,
    slot = "any",
    excludeIds = [],
  } = options;
...
export function buildCookunitySevenDayMealPlan(keyword: string, secondaryKeywords: string[] = []) {
  ...
  const days: CookunityMealPlanDay[] = [];
...
export function flattenMealPlanDays(keyword: string, secondaryKeywords: string[] = []) {
